...
def prepare_inputs(payload: dict) -> dict:
    ...
    trinmed = trim_messages(raw_history,
                            token_counter=len,
                            max_tokens=2,
                            strategy="last",
                            start_on="human",
                            include_system=True,
                            allow_partial=False,)
    return {"input": payload.get("input",""), "history": trinmed}
...
resp1 = conversational_chain.invoke({"input":"Hello my name is Thiago. Reply only with 'OK' and do not mention my name."}, config=config)
